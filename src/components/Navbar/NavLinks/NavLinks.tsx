...
const NavLinks:React.FC<NavLinksProps> = ({ isMobile }) => { 
    ...
    const [activeLink, setActiveLink] = useState<number | null>(null); // For desktop 
    const [activeMobileLink, setActiveMobileLink] = useState<number | null>(null); // For mobile click 
    const [timeoutId, setTimeoutId] = useState<NodeJS.Timeout | null>(null); // timeout desktop dropdown
        ...
        <div className='flex flex-col lg:flex-row gap-4'>
            {links.map((link, index) => (
                ...
                >
                    {/* Main Link */}
                    <div 
                        className={`text-gray-800 font-black cursor-pointer hover:text-blue-600 text-left px-3 py-2 flex items-center justify-between ${isMobile ? 'text-lg' : ''}`}
